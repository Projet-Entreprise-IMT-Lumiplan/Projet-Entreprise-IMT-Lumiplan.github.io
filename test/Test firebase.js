...
let Id = 1;
let toolName = get(child(dbRef, 'dataOutils/${Id}/Outils')).then((snapshot) => {
    if (snapshot.exists()) {
...
for (let i = 0; i < logiciels.length; i++) {
  ...
  pieceElement.setAttribute("data-bs-logiciel", i);
  ...
  let toolName = get(child(dbRef, 'dataOutils/${toolId}/Outils')).then((snapshot) => {
    if (snapshot.exists()) {
      console.log(snapshot.val());
    } else {
      console.log("No data available");
  ...
  imageElement.src = "./Frames_Icones/" + toolName +".png";
...
exampleModal.addEventListener("show.bs.modal", (event) => {
  ...
  pathData = 'dataOutils/${toolId}';
  let logiciels = get(child(dbRef, pathDataBase)).then((snapshot) => {
    if (snapshot.exists()) {
      console.log(snapshot.val());
    } else {
      console.log("No data available");
